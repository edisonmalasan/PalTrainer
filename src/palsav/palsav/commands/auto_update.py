...
def get_paltrainer_root():
    return Path(__file__).resolve().parent.parent.parent.parent.parent
def get_src_dir():
    return get_paltrainer_root() / 'src'
...
def run_convert(sav_path):
    ...
    proc = subprocess.run([sys.executable, '-m', 'palsav.commands.convert', str(sav_path), '--to-json', '-o', output, '--force'], capture_output=True, text=True, cwd=get_paltrainer_root(), env=get_subprocess_env(), timeout=120)
    return (proc, output)
def run_roundtrip(sav_path):
    ...
    proc_json = subprocess.run([sys.executable, '-m', 'palsav.commands.convert', str(sav_path), '--to-json', '-o', temp_json, '--force'], capture_output=True, text=True, cwd=get_paltrainer_root(), env=env, timeout=120)
    if proc_json.returncode != 0:
        return ('json_fail', proc_json.stderr)
    proc_sav = subprocess.run([sys.executable, '-m', 'palsav.commands.convert', temp_json, '--from-json', '-o', temp_sav, '--force'], capture_output=True, text=True, cwd=get_paltrainer_root(), env=env, timeout=120)
