...
if _cli.env_flag('PALTRAINER_NO_GUI'):
    ...
    try:
        from PyQt6.QtWidgets import QApplication, QWidget, QVBoxLayout, QLabel, QProgressBar, QGraphicsOpacityEffect, QFrame, QHBoxLayout, QSpacerItem, QSizePolicy
        from PyQt6.QtGui import QPixmap
        from PyQt6.QtCore import Qt, QPropertyAnimation, QEasingCurve, QTimer, QObject, pyqtSignal, pyqtSlot, qInstallMessageHandler
        GUI_AVAILABLE = True
    except Exception:
        GUI_AVAILABLE = False
if GUI_AVAILABLE:
...
if GUI_AVAILABLE:
    ...
    def _load_logo_pixmap() -> Optional[QPixmap]:
        logo_path = _find_resource('resources/assets/branding/logo.png', 'resources/assets/branding/PalTrainer_Blue.png')
        if logo_path:
                ...
                with open(REQ_FILE, 'r') as f:
                    for line in f:
                        ...
                        if line and (not line.startswith('#')):
                            ...
                            elif 'pyqt6-essentials' not in line and (not line.startswith('./')) and (not line.startswith('..')):
