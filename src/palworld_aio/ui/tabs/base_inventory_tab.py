...
class GuildItemPickerDialog(BaseDialog):
    ...
    def _on_item_clicked(self, item: QListWidgetItem):
        ...
        self.info_label.setStyleSheet('color: #2DD4BF; font-weight: bold; font-size: 13px; padding: 2px;')
...
class GuildStructurePickerDialog(BaseDialog):
    ...
    def _on_delete_structure(self):
        ...
        if not selected_guilds:
            show_warning(self, t('base_inventory.no_guilds_selected') if t else 'No guilds selected', t('base_inventory.no_guilds_selected') if t else 'Please select at least one guild.')
            return
        from palworld_aio.inventory.base_inventory_manager import count_structures_for_removal
        ...
        msg_box.setText(t('ui.safety.guild_structures_confirm',
                          count=affected, name=structure_name,
                          default='Delete {count} {name} structures from the selected guilds? Associated inventory and Pal data may also be removed. This cannot be undone.'))
...
class ReplaceStructureDialog(BaseDialog):
    ...
    def _populate_left(self):
        ...
        if not self.structure_entries:
            ...
            item.setForeground(QColor('#A69F94'))
    ...
    def _populate_right(self, family, source_element, source_name):
        ...
        for element, fam, asset in entries:
            ...
            list_item.setForeground(QColor('#ECE7E0'))
            list_item.setSizeHint(QSize(90, 90))
...
_CONTAINER_NAME_OVERRIDES = {
    'commondropitem3d': 'Dropped Items',
    'commondropitem3d_sk': 'Dropped Items',
}
...
def _friendly_container_display_name(container_info):
    ...
    first, then the resolved structure name; raw asset ids get the
    3D/Common/Drop/Item casing artifacts stripped. Duplicate indexing is
    applied by the caller."""
    asset = str(container_info.get('map_object_id') or container_info.get('type') or '')
    override = _CONTAINER_NAME_OVERRIDES.get(asset.lower())
    if override:
        return override
    name = str(container_info.get('name') or '')
    if name and name != asset:
        return name
    if not asset:
        return name or 'Container'
    words = []
    for token in re.split(r'[_\s]+', asset):
        for part in re.findall(r'[A-Z]+(?![a-z])|[A-Z][a-z0-9]*|^[a-z0-9]+', token):
            if part.lower() not in _CONTAINER_NAME_ARTIFACTS:
                words.append(part)
    return ' '.join(words) if words else asset
...
class ContainerListWidget(QTreeWidget):
    ...
    def _show_context_menu(self, position):
        ...
        if item and item.data(0, Qt.UserRole) is not None:
            ...
            key = popup.exec(self.viewport().mapToGlobal(position))
            if key == 'add_item':
...
class ContainerInfoWidget(QWidget):
    """Compact stats strip for the selected container (modernize-tab-ui 2.3).

    The list selection card already shows image, name, slot count, and id;
    this widget renders only the detail the card lacks (item / empty-slot
    counts) and hides itself when no container is selected so the selection
    summary never appears twice in the left column."""
    def __init__(self, parent=None):
        super().__init__(parent)
    ...
    def _setup_ui(self):
        ...
        layout.addWidget(self.empty_slots_label)
        layout.addStretch()
        self._update_styles()
        self.setVisible(False)
    def set_container_info(self, container_info):
        self.container_info = container_info
        self.setVisible(container_info is not None)
...
class _BasePalIcon(QFrame):
    ...
    def contextMenuEvent(self, event):
        if self.pal_data:
            ...
            key = popup.exec(event.globalPos())
            if key is None:
                return
            if key == 'boss':
    ...
    def _build(self):
        ...
        level_lbl.setStyleSheet('color: #F59E0B; font-size: 11px; font-weight: bold; background: rgba(0,0,0,0.7); border: 1px solid rgba(245,158,11,0.25); border-radius: 3px; padding: 0 3px;')
...
class BasePalsContentWidget(QFrame):
    ...
    def _setup_ui(self):
        ...
        self.stats_label.setStyleSheet('font-weight: bold; font-size: 12px; color: #F59E0B; padding: 2px 4px;')
    ...
    def _on_pal_right_clicked(self, idx, action):
        ...
        if action == 'boss_toggle':
            ...
            rename_lbl.setStyleSheet('font-size: 11px; font-weight: 600; color: #F59E0B; background: transparent; border: none;')
            il.addWidget(rename_lbl)
            rename_edit = QLineEdit()
            rename_edit.setPlaceholderText(pal_name)
            rename_edit.setStyleSheet('QLineEdit { background: rgba(0,0,0,0.4); color: #ECE7E0; border: 1px solid rgba(245,158,11,0.2); border-radius: 4px; padding: 6px 10px; font-size: 12px; } QLineEdit:focus { border-color: #F59E0B; }')
            il.addWidget(rename_edit)
            list_lbl = QLabel(t('edit_pals.select_pals_to_sync'))
            list_lbl.setStyleSheet('font-size: 10px; font-weight: 600; color: #F59E0B; background: transparent; border: none; padding: 2px 0;')
            ...
            info_lbl.setStyleSheet('font-size: 11px; color: #A69F94; background: transparent; border: none; padding: 4px 0;')
...
class BaseInventoryTab(QWidget):
    ...
    def refresh_labels(self):
        ...
        if hasattr(self, 'base_button'):
            ...
            if self._current_base_id:
                self.base_button.setToolTip(str(self._current_base_id))
            else:
        ...
        if hasattr(self, 'item_button'):
            ...
            set_picker_selected(self.item_button, bool(self.selected_item_id))
        ...
        if hasattr(self, 'structure_button'):
            ...
            set_picker_selected(self.structure_button, bool(self.selected_structure_asset))
    ...
    def _show_base_popup(self):
        ...
        for base in self._bases_data:
            if self._current_base_id and str(base['id']) == str(self._current_base_id):
                continue
            # modernize-tab-ui 2.1: "Base N" rows; GUID in tooltip.
            item = QListWidgetItem(f"Base {self._bases_data.index(base) + 1}")
            item.setToolTip(str(base['id']))
    ...
    def _on_base_changed(self, base_id, *, publish: bool = True):
        ...
        # modernize-tab-ui 2.1: friendly "Base N" label; GUID stays in tooltip.
        if base:
            self._current_base_name = f"Base {self._bases_data.index(base) + 1}"
        else:
    ...
    def _on_item_action_selected(self, item_id: str, action: str, guild_ids: list=None):
        ...
        if action == 'find':
            if item_name:
                ...
                self.clear_item_button.setVisible(True)
                set_picker_selected(self.item_button, True)
            ...
            preview = remove_item_from_guilds(
                item_id, percentage, guild_ids, preview_only=True)
            if not preview['removed']:
                self._show_warning(t('base_inventory.no_items_removed'))
                return
            if not show_question(
                self, t('base_inventory.remove_from_guilds'),
                t('ui.safety.guild_items_confirm', count=preview['removed'],
                  containers=preview['containers_affected'], item=item_name,
                  default='Change {count} {item} units across {containers} containers? This cannot be undone from the editor.'),
            ):
                return
            ...
            if result and result.get('removed', 0) > 0:
                recorder = getattr(self._main_window, 'record_pending_change', None)
                if callable(recorder):
                    recorder('Reduce guild item quantities',
                             affected_count=result['removed'], high_risk=True)
                    self._suppress_next_auto_save_journal = bool(self.manager.inventory_container)
            ...
            from palworld_aio.inventory.base_inventory_manager import remove_item_from_guilds
            preview = remove_item_from_guilds(
                item_id, None, guild_ids, preview_only=True)
            if not preview['removed']:
                self._show_warning(t('base_inventory.no_items_removed'))
                return
            if not show_question(
                self, t('base_inventory.confirm_remove_all'),
                t('ui.safety.guild_items_confirm', count=preview['removed'],
                  containers=preview['containers_affected'], item=item_name,
                  default='Change {count} {item} units across {containers} containers? This cannot be undone from the editor.'),
            ):
    ...
    def _clear_item_filter(self):
        ...
        self.item_button.setText(t('base_inventory.all_items') if t else 'All Items')
        self.clear_item_button.setVisible(False)
        set_picker_selected(self.item_button, False)
        self._reset_filters()
    ...
    def _on_structure_action_selected(self, structure_asset: str, action: str, guild_ids: list=None):
        if action == 'find':
            ...
            set_picker_selected(self.structure_button, True)
            ...
            if result and result.get('removed', 0) > 0:
                recorder = getattr(self._main_window, 'record_pending_change', None)
                if callable(recorder):
                    recorder('Delete guild structures',
                             affected_count=result['removed'], high_risk=True)
                    self._suppress_next_auto_save_journal = bool(self.manager.inventory_container)
                guilds_count = len(guild_ids) if guild_ids else 0
