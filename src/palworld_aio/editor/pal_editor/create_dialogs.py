...
def _show_learned_moves_dialog(raw, parent):
    ...
    search_edit.setStyleSheet('QLineEdit { background: rgba(0,0,0,0.3); color: #ECE7E0; border: 1px solid rgba(245,158,11,0.2); border-radius: 4px; padding: 4px 8px; font-size: 11px; } QLineEdit:focus { border-color: rgba(245,158,11,0.5); }')
    il.addWidget(search_edit)
    scroll = QScrollArea()
    scroll.setWidgetResizable(True)
    scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
    scroll.setStyleSheet('QScrollArea { background: transparent; border: 1px solid rgba(245,158,11,0.12); border-radius: 4px; }')
    scroll_ct = QWidget()
    ...
    if not mw_list:
        ...
        for idx, skill_val in enumerate(mw_list):
            ...
            slot = SkillSlotFrame()
            slot.setStyleSheet('SkillSlotFrame { background: rgba(0,0,0,0); border: 1px solid rgba(245,158,11,0.08); border-radius: 3px; } SkillSlotFrame:hover { background: rgba(245,158,11,0.06); border: 1px solid rgba(245,158,11,0.2); }')
            ...
            name_lbl.setStyleSheet('font-size: 11px; font-weight: 600; color: #ECE7E0; background: transparent; border: none;')
            ...
            if skill_elem:
                ...
                if ep:
                    ...
                    elem_badge.setStyleSheet('background: rgba(255,255,255,0.04); border: 1px solid rgba(236,231,224,0.06); border-radius: 2px; padding: 1px; margin: 0px;')
                else:
    ...
    learn_all_btn.setStyleSheet('QPushButton { background: rgba(45,212,191,0.15); color: #2DD4BF; border: 1px solid rgba(45,212,191,0.3); border-radius: 4px; padding: 6px 20px; font-size: 12px; font-weight: 700; } QPushButton:hover { background: rgba(45,212,191,0.25); color: #FFFFFF; }')
    ...
    add_skill_btn.setStyleSheet('QPushButton { font-size: 14px; font-weight: 700; color: #F59E0B; background: rgba(245,158,11,0.1); border: 1px solid rgba(245,158,11,0.25); border-radius: 4px; padding: 0px; } QPushButton:hover { background: rgba(245,158,11,0.2); color: #FFFFFF; }')
    ...
    close_btn.setStyleSheet('QPushButton { background: rgba(245,158,11,0.1); color: #F59E0B; border: 1px solid rgba(245,158,11,0.25); border-radius: 4px; padding: 6px 20px; font-size: 12px; font-weight: 600; } QPushButton:hover { background: rgba(245,158,11,0.2); color: #FFFFFF; }')
...
class BulkSyncPalDialog(FramelessDialog):
    def __init__(self, pal_item, pal_editor, parent=None, candidates=None):
        ...
        name_lbl.setStyleSheet('font-size: 14px; font-weight: 700; color: #ECE7E0; background: transparent; border: none;')
        ...
        pal_list_label.setStyleSheet('font-size: 10px; font-weight: 600; color: #F59E0B; background: transparent; border: none; padding: 2px 0;')
        ...
        pal_scroll.setStyleSheet('QScrollArea { background: transparent; border: 1px solid rgba(245,158,11,0.12); border-radius: 4px; }')
        ...
        preview_label.setStyleSheet('font-size: 10px; font-weight: 600; color: #F59E0B; background: transparent; border: none; padding: 2px 0;')
        ...
        sel_all_btn.setStyleSheet('QPushButton { background: rgba(245,158,11,0.08); color: #F59E0B; border: 1px solid rgba(245,158,11,0.2); border-radius: 4px; padding: 6px 12px; font-size: 11px; font-weight: 600; } QPushButton:hover { background: rgba(245,158,11,0.15); color: #FFFFFF; }')
        ...
        sel_none_btn.setStyleSheet('QPushButton { background: rgba(255,255,255,0.04); color: #9CA3AF; border: 1px solid rgba(236,231,224,0.10); border-radius: 4px; padding: 6px 12px; font-size: 11px; font-weight: 600; } QPushButton:hover { background: rgba(255,255,255,0.08); color: #FFFFFF; }')
...
class BulkSyncAllDialog(FramelessDialog):
    def __init__(self, pal_item, pal_editor, parent=None, candidates=None):
        ...
        header.setStyleSheet('font-size: 12px; font-weight: 700; color: #ECE7E0; background: transparent; border: none;')
        il.addWidget(header)
        if not self._external_candidates:
            src_row = QHBoxLayout()
            src_row.setSpacing(8)
            src_lbl = QLabel(t('edit_pals.bulk_sync_sources'))
            src_lbl.setStyleSheet('font-size: 11px; font-weight: 600; color: #F59E0B; background: transparent; border: none;')
...
class BulkSpeciesDialog(FramelessDialog):
    def __init__(self, pal_editor, mode='clone', parent=None, external_pals=None):
        ...
        if not self._external_pals:
            ...
            src_lbl.setStyleSheet('font-size: 11px; font-weight: 600; color: #F59E0B;')
            src_row.addWidget(src_lbl)
            self._party_chk = ToggleCheckBtn(t('edit_pals.party'))
            self._party_chk.setChecked(True)
            self._party_chk.toggled.connect(self._on_source_toggle)
            ...
            self._slot_label.setStyleSheet('font-size: 10px; color: #A69F94;')
        ...
        self._species_list.setStyleSheet('QListWidget { color: #ECE7E0; background: transparent; border: none; } QListWidget::item { color: #ECE7E0; } QListWidget::item:selected { color: #ffffff; background: rgba(245,158,11,0.15); } QListWidget::item:hover { color: #f1f5f9; background: rgba(245,158,11,0.08); }')
        ...
        self._count_label.setStyleSheet('font-size: 11px; color: #A69F94; background: transparent; border: none;')
        controls_row.addWidget(self._count_label)
        controls_row.addStretch()
        if mode == 'clone':
            qty_lbl = QLabel(t('edit_pals.bulk_qty'))
            qty_lbl.setStyleSheet('font-size: 11px; color: #A69F94; background: transparent; border: none;')
        ...
        self._apply_btn.setStyleSheet('QPushButton { background: rgba(45,212,191,0.15); color: #2DD4BF; border: 1px solid rgba(45,212,191,0.3); border-radius: 4px; padding: 6px 20px; font-size: 12px; font-weight: 700; } QPushButton:hover { background: rgba(45,212,191,0.25); color: #FFFFFF; } QPushButton:disabled { background: rgba(255,255,255,0.03); color: rgba(255,255,255,0.2); border: 1px solid rgba(236,231,224,0.06); }')
        ...
        self._footer_label.setStyleSheet('font-size: 10px; color: #A69F94; padding: 2px 4px;')
    ...
    def _update_footer(self):
        if self.mode == 'clone' and self._selected_cid:
            ...
            color = '#2DD4BF' if total_requested <= free else '#F87171'
...
class FoodPickerDialog(FramelessDialog):
    ...
    def _setup_ui(self):
        ...
        self._search_edit.setStyleSheet('QLineEdit { background: rgba(0,0,0,0.4); color: #ECE7E0; border: 1px solid rgba(245,158,11,0.2); border-radius: 4px; padding: 6px 10px; font-size: 12px; } QLineEdit:focus { border-color: #F59E0B; }')
    ...
    def _populate(self):
        ...
        for food_id, fb in sorted(fb_map.items()):
            ...
            eff_lbl.setStyleSheet('font-size: 10px; color: #A69F94; background: transparent; border: none;')
            rl.addWidget(eff_lbl, 1)

            cat_lbl = QLabel(cat_label)
            cat_lbl.setStyleSheet('font-size: 10px; color: #F59E0B; background: transparent; border: none;')
            cat_lbl.setFixedWidth(90)
            rl.addWidget(cat_lbl)

            dur_lbl = QLabel(dur_str)
            dur_lbl.setStyleSheet('font-size: 10px; color: #A69F94; background: transparent; border: none;')
...
class CloneBulkDialog(FramelessDialog):
    def __init__(self, pals, free_slots, parent=None):
        ...
        head.setStyleSheet('font-size: 12px; font-weight: 600; color: #ECE7E0; background: transparent; border: none;')
        outer.addWidget(head)

        all_row = QHBoxLayout()
        all_row.setSpacing(6)
        all_lbl = QLabel(t('edit_pals.clone_bulk_set_all'))
        all_lbl.setStyleSheet('font-size: 11px; color: #A69F94; background: transparent; border: none;')
        ...
        apply_all.setStyleSheet('QPushButton { background: rgba(245,158,11,0.12); color: #F59E0B; border: 1px solid rgba(245,158,11,0.25); border-radius: 4px; padding: 3px 10px; font-size: 11px; font-weight: 600; } QPushButton:hover { background: rgba(245,158,11,0.25); color: #FFFFFF; }')
        ...
        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        scroll.setStyleSheet('QScrollArea { background: transparent; border: 1px solid rgba(245,158,11,0.15); border-radius: 6px; }')
        listw = QWidget()
        ...
        self._total_lbl.setStyleSheet('font-size: 11px; font-weight: 700; color: #F59E0B; background: transparent; border: none;')
        ...
        cancel.setStyleSheet('QPushButton { background: rgba(255,255,255,0.05); color: #9CA3AF; border: 1px solid rgba(236,231,224,0.10); border-radius: 4px; padding: 6px 16px; font-size: 12px; font-weight: 600; } QPushButton:hover { background: rgba(236,231,224,0.10); color: #FFFFFF; }')
        cancel.clicked.connect(self.reject)
        btn_row.addWidget(cancel)
        self._ok = QPushButton(t('edit_pals.clone_bulk_apply'))
        self._ok.setCursor(Qt.PointingHandCursor)
        self._ok.setStyleSheet('QPushButton { background: rgba(45,212,191,0.15); color: #2DD4BF; border: 1px solid rgba(45,212,191,0.3); border-radius: 4px; padding: 6px 20px; font-size: 12px; font-weight: 700; } QPushButton:hover { background: rgba(45,212,191,0.25); color: #FFFFFF; } QPushButton:disabled { background: rgba(255,255,255,0.03); color: rgba(255,255,255,0.2); border: 1px solid rgba(236,231,224,0.06); }')
    ...
    def _build_row(self, raw):
        ...
        name_lbl.setStyleSheet('font-size: 11px; color: #ECE7E0; background: transparent; border: none;')
        row.addWidget(name_lbl, 1)
        lv_lbl = QLabel(t('edit_pals.clone_bulk_level', level=level))
        lv_lbl.setStyleSheet('font-size: 10px; color: #A69F94; background: transparent; border: none;')
    ...
    def _update_total(self):
        ...
        color = '#F87171' if over else '#F59E0B'
