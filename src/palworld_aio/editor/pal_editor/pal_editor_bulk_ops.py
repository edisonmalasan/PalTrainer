from PyQt6.QtWidgets import QDialog, QHBoxLayout, QLabel, QLineEdit, QPushButton, QScrollArea, QSizePolicy, QVBoxLayout, QWidget
...
class BulkOperationMixin:
    ...
    def _bulk_rename_pal(self, sender):
        ...
        rename_edit.setStyleSheet(f'QLineEdit {{ background: rgba(0,0,0,0.4); color: {_P["text"]}; border: 1px solid {_P["info_border"]}; border-radius: 4px; padding: 6px 10px; font-size: 12px; }} QLineEdit:focus {{ border-color: {_P["info"]}; }}')
        il.addWidget(rename_edit)
        list_lbl = QLabel(t('edit_pals.select_pals_to_sync'))
        list_lbl.setStyleSheet(f'font-size: 10px; font-weight: 600; color: {_P["info"]}; background: transparent; border: none; padding: 2px 0;')
        il.addWidget(list_lbl)
        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        scroll.setStyleSheet('QScrollArea { background: transparent; border: 1px solid rgba(147,183,221,0.10); border-radius: 4px; }')
        inner_w = QWidget()
