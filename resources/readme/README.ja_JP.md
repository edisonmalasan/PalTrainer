...
### **スタンドアロン バージョンを [GitHub Releases](https://github.com/edisonmalasan/PalTrainer/releases/latest)** からダウンロードします
...
PalTrainer (PalTrainer) は、Palworld セーブ ファイルを検査および編集するための高速なオールインワン デスクトップ アプリケーションです。 Python と PyQt6 で構築されており、ゲームの圧縮保存形式を直接読み書きします。ゲーム MOD は必要ありません。

専用サーバーの管理、Co-op サーバーと専用サーバー間の移行、放棄されたデータのクリーンアップ、または個々の Pals の微調整が必​​要な場合でも、PalTrainer はそのすべてに単一の統合インターフェイスを提供します。

### ハイライト

- **クロスプラットフォーム** — **Windows**、**Linux**、**macOS** 用の事前構築済みバイナリ。
- **高速ネイティブ解析** — [`palsav`](src/palsav) エンジンを搭載した、利用可能な中で最も高速な保存ファイル リーダーの 1 つ。
- **ビジュアルマップ** — ベース/プレイヤーマーカー、除外ゾーン、座標調整を備えたインタラクティブなワールドマップ。
...
| **GamePass → Steam** | 変換Xbox/GamePass セーブを Steam 形式に変換する |
...
事前に構築されたバイナリは、[GitHub Releases](https://github.com/edisonmalasan/PalTrainer/releases/latest) 以降の 3 つの主要なプラットフォームすべてで利用できます。

|プラットフォーム |ダウンロード |要件 |
|----------|----------|--------------|
| **Windows** | `PalTrainer-*.exe` | Windows 10/11、[VC++ Redistributable](https://learn.microsoft.com/en-us/cpp/windows/latest-supported-vc-redist?view=msvc-170) (2015–2022) |
| **Linux** | `PalTrainer-*-linux.AppImage` |最新のディストリビューション |
| **macOS** | `PalTrainer-*-macos.dmg` | macOS 12+ (モントレー以降) |

[community releases](https://github.com/edisonmalasan/PalTrainer/releases) でもご利用いただけます。
...
> **Linux:** ファイルを実行可能としてマークする必要がある場合があります: `chmod +x PalTrainer-*-linux.AppImage`

> **macOS:** Gatekeeper がアプリをブロックする場合は、最初に右クリック → **開く**、または `xattr -d com.apple.quarantine /path/to/app` を実行します。

### ソースから (すべてのプラットフォーム)

PalTrainer は依存関係の管理に [`uv`](https://docs.astral.sh/uv/) を使用します。起動スクリプトは自動的に仮想環境を作成し、すべてをインストールします。

**前提条件:** [Python 3.11+](https://www.python.org/) および [uv](https://docs.astral.sh/uv/getting-started/installation/)。

```bash
git clone https://github.com/edisonmalasan/PalTrainer.git
cd PalTrainer
uv run start.py
```
...
PalTrainer では、セーブ用にマップ全体 (すべてのファストトラベル ポイント) のロックを解除できます。

1. PalTrainer でセーブデータをロードします。
...
**仕組み:** 協力プレイでは、ホスト プレーヤーに `0001.sav` を使用します。専用サーバーはそうではありません。すべてのプレーヤーは通常の UID を持っています。ホストがコピーではなく 2 つのプレーヤー ファイル (シートのトレードなど) を **交換** 保存する問題を修正しました。 `0001.sav` の協力キャラクターはサーバーのスロットに交換されます。
...
3. **協力キャラクターをサーバー スロットに交換します。**
   - PalTrainer → **ツール** > **ホスト保存の修正** を開きます。
   - サーバーの `Level.sav` を参照します。
   - **ソース プレイヤー**: 協力キャラクター (`0001.sav` のキャラクター — ホストとしてリストされているもの) を選択します。
...
1. **サーバーの保存をローカル PC にコピーします。**
   ...
   - そこから `Level.sav` と `Players` フォルダーをコピーします。
...
1. PalTrainer → **ツール** → **キャラクター転送** を開きます。
...
- **Linux:** `chmod +x PalTrainer-*-linux.AppImage` を実行可能としてマークします。
...
PalTrainer は 2 つのビルド パスをサポートします。 CI/CD パイプラインは、クロスプラットフォーム リリース バイナリに Nuitka を使用します。 cx_Freeze は、ローカル Windows インストーラーに使用されます。
...
出力は `dist/` に送られます。
- Windows → `dist/PalTrainer-*.exe`
- Linux → `dist/PalTrainer-*-linux`
- macOS → `dist/PalTrainer.app` → `.dmg` としてパッケージ化
...
これにより、プロジェクト ルートに `PalTrainer_standalone_v{version}.zip` が作成されます。
...
- **GitHub 問題:** [Report a bug](https://github.com/edisonmalasan/PalTrainer/issues)
- **community releases:** [Download & discuss](https://github.com/edisonmalasan/PalTrainer/releases)
...
**[Pylar](https://github.com/edisonmalasan)** — すべてを始めた男。このツールのすべての行は、彼のビジョンと、保存エンジン、GUI、および毎日使用する機能に対するたゆまぬ努力をたどります。
...
**[oMaN-Rod](https://github.com/oMaN-Rod)** — このプロジェクトのフォーク元となったオリジナルの保存パーサーを提供しました。 Palworld 保存形式のクラックに関する彼の基礎的な作業がなければ、これは存在しませんでした。フォークにより、彼のパーサーは合理化され、簡素化され、現在の PalTrainer になりました。
...
- **すべての寄稿者およびコミュニティ メンバー** — PR を送信したことがある方、Discord の質問に回答したことがある方、単に PalTrainer について友人に話したことがあることにかかわらず – ありがとうございます。

---

<div align="center">

![Divider](../assets/branding/PalTrainer_readme_divider.png)
