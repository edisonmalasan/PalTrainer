...
- **Last completed task:** 9.4 — High-risk Save Changes and immediate player/DPS mutations now create a recovery copy before writing; routine saves offer one when needed. Backup failure aborts the write. Save and restore failures report whether the original changed or its state is uncertain, and give the recovery path when available. Restore safety snapshots appear in Backup state and Activity. Load-time backup state reflects the actual preference and result.
- **Verification:** The Phase 8 focused verification and ten-state synthetic render inspection were completed in the preceding session. Phase 9.1 passed 138 focused tests; Phase 9.3's replacement flow passed its focused selection. Phase 9.2 passed 132 focused tests. For 9.4, the combined save session, manager, backup catalog, MainWindow, repair dialog, player item, localization, and resource integrity selection passed (126 tests) using an isolated Python environment with local pure Python `palsav`. Compilation, parsing all 11 localization JSON files, `git diff --check`, and strict OpenSpec validation passed. Relevant `npx --yes pyright` reported 81 errors, including unresolved PyQt6 imports and existing type issues, so Pyright is not claimed as passing. `uv run pytest -c tests/pytest.ini` stopped before collection because the `palooz` extension requires unavailable Microsoft Visual C++ 14.0 or newer.
...
- **Next task:** 9.5 — Assess Palworld process detection reliability and implement or document the running-game warning.
