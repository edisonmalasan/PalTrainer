...
- **Last completed task:** 9.1 — Added a pending-change journal tied to the workspace context, explicit Saved/Unsaved/Saving/Failed/Read Only/Backup Recommended header states, change-summary details, and save-failure preservation. Player and Base Inventory, player stats, Pal details/collections, and existing mutation refresh paths now report in-memory changes; a legacy dirty-flag bridge records older edit batches without offering unsupported Undo. Load and restore refreshes no longer create false pending edits.
- **Verification:** The Phase 8 focused verification and ten-state synthetic render inspection were completed in the preceding session. In this session, the Phase 9.1 MainWindow/shell/context/header/journal/state/inventory/Pal selection passed (138 tests) using an isolated Python tool environment with the local pure Python `palsav` source. The later Phase 9.2 close-guard, journal callback, and bulk item confirmation selection passed (46 tests). Compilation, localization JSON parsing, strict OpenSpec validation, and diff checks passed. A focused Pyright check of the new presentation model and header/shell modules reported zero errors using an isolated dependency environment. The normal `uv run pytest` command could not start: Python 3.14 was selected and the native `palooz` extension requires unavailable MSVC Build Tools. Repository-wide Pyright is not claimed as passing.
...
- **Next task:** 9.2 — Complete review and save/revert/discard guards across remaining risky workflows. The pending-change menu now offers Undo/Redo only for callbacks that exist, close Save/Discard/Cancel preserves work on failure or cancellation, and bulk item Add discloses item quantity and affected player count before execution.
